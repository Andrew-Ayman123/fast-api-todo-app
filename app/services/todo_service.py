...
class TodoService:
    ...
    async def create_todo_list(self, todo_data: TodoListCreateRequest, user_id: uuid.UUID) -> TodoListModel:
        ...
        Args:
            ...
            user_id (uuid.UUID): ID of the user creating the todo list.
    ...
    async def get_todo_list_by_id(self, todo_id: uuid.UUID, user_id: uuid.UUID) -> TodoListModel:
        ...
        Args:
            todo_id (uuid.UUID): ID of the todo list to retrieve.
            user_id (uuid.UUID): ID of the user requesting the todo list.
    ...
    async def get_all_todo_lists_without_items(
        ...
        Args:
            user_id (uuid.UUID): ID of the user whose todo lists to retrieve.
    ...
    async def update_todo_item(
        ...
        """
        item = await self.todo_repository.update_todo_list_item(todo_id, item_id, item_data, user_id)
        if not item:
    ...
    async def delete_todo_list_item(self, todo_id: uuid.UUID, item_id: uuid.UUID, user_id: uuid.UUID) -> None:
        ...
        """
        success = await self.todo_repository.delete_todo_list_item(todo_id, item_id, user_id)
        if not success:
            raise TodoListItemNotFoundError(todo_id, item_id)
    ...
    async def update_many_todo_lists(self, updates: list, user_id: uuid.UUID) -> list[TodoListModel]:
    ...
    async def delete_many_todo_list_items(
        self, todo_id: uuid.UUID, item_ids: list[uuid.UUID], user_id: uuid.UUID,
    ) -> None:
        """Delete multiple items from a user's todo list.

        Args:
            todo_id (uuid.UUID): ID of the todo list containing the items to delete.
            item_ids (list[uuid.UUID]): List of item IDs to delete.
            user_id (uuid.UUID): ID of the user deleting the items.
